...
function saveThemePreference(layout) {
  if (typeof Storage !== 'undefined' && layout) {
    localStorage.setItem('theme', layout);
  }
}

// Modifier la fonction layout_change pour inclure la sauvegarde
const original_layout_change = layout_change;
layout_change = function(layout) {
  original_layout_change(layout); // garde le comportement original
  saveThemePreference(layout);    // enregistre le thème choisi
};

// Appliquer le thème sauvegardé au rechargement
document.addEventListener('DOMContentLoaded', function() {
